        ...
        try {
            ...
            if ($user->email_notifications) {
                try {
                    ...
                    Log::info("Email sent to {$user->email} for project {$project->name} (Days remaining: {$daysRemaining})");
                } catch (\Exception $e) {
                    Log::error("Failed to send project due reminder email to {$user->email}: " . $e->getMessage());
                }
            } else {
                Log::info("Email notifications disabled for user {$user->id}");
            }
        } catch (\Exception $e) {
            Log::error("Error in sendProjectDueReminder: " . $e->getMessage());
        }
    }
